...
namespace CanamLiveFA.DAL
{
    public class User : BaseDal
    {
        ...
        private static DO.User PopulateUser(SqlDataReader sqlReader)
        {
            ...
            double bidFeesPayable = DAL.Bids.BidFeesPayable(userId);
            double bidFeesReceivable = DAL.Bids.BidFeesReceivable(userId);
            bool commissioner = GetBooleanValue(sqlReader["UCommissioner"]);
            userObj.PopulateUser(userId, userName, team, bidFeesPayable, bidFeesReceivable, password, commissioner);
            return userObj;
        }
    }
}